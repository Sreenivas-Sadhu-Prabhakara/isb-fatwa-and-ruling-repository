...
| POST | `/v1/shariah-ruling-directory-entry/initiate` | Initiate |
| GET | `/v1/shariah-ruling-directory-entry` | Retrieve (list) |
| GET | `/v1/shariah-ruling-directory-entry/{crId}/retrieve` | Retrieve |
| PUT | `/v1/shariah-ruling-directory-entry/{crId}/update` | Update |
| PUT | `/v1/shariah-ruling-directory-entry/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/shariah-ruling-directory-entry/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-fatwa-and-ruling-repository:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-fatwa-and-ruling-repository ./helm -n isb-governance
```

Exposed through the platform Gateway at path prefix `/isb-fatwa-and-ruling-repository` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
